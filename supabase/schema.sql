...
create table if not exists events (
  ...
  user_id uuid references auth.users(id) on delete cascade not null,
  title text not null,
  event_date date not null,          -- e.g. '2026-05-15'
  event_time text,                   -- e.g. '09:30 AM'
  location text,
  event_type text default 'other',   -- 'exam' | 'class' | 'deadline' | 'other'
  course_id uuid references courses(id) on delete set null,
  created_at timestamptz default now()
);

-- Notifications (admin-published, shown to all users)
create table if not exists notifications (
  id uuid default gen_random_uuid() primary key,
  title text not null,
  body text not null,
  category text default 'update',    -- 'update' | 'education' | 'system'
...
-- notifications is intentionally readable by all authenticated users (no RLS needed for read)
...
create policy "Users can view their own research documents"
  on public.research_documents for select
  using (auth.uid() = user_id);

create policy "Users can insert their own research documents"
...
-- Add extracted_text column to documents for AI Context
