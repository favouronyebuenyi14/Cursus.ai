...
if (!profile) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <aside className="fixed left-0 top-0 z-40 hidden h-screen w-64 flex-col bg-slate-50 p-4 md:flex">
        <div className="mb-8 px-2">
          <h1 className="text-2xl font-bold tracking-tighter text-blue-900">Cursus</h1>
        ...
        <div className="mt-4 px-2">
          <button onClick={() => setCoursesOpen(!coursesOpen)}
            className="w-full flex items-center justify-between rounded-xl bg-slate-100 px-4 py-3 text-slate-700 transition-colors hover:bg-slate-200">
          ...
          {coursesOpen && (
            <div className="mt-2 space-y-1">
              <button onClick={() => setSelectedCourse(null)}
                className={`w-full rounded-lg px-3 py-2 text-left text-xs transition-colors ${!selectedCourse ? 'text-blue-700' : 'text-slate-500 hover:text-blue-700'}`}>
                All courses
              </button>
              {courses.map(c => (
                <button key={c.id} onClick={() => setSelectedCourse(c.id)}
                  className={`w-full truncate rounded-lg px-3 py-2 text-left text-xs transition-colors ${selectedCourse === c.id ? 'text-blue-700' : 'text-slate-500 hover:text-blue-700'}`}>
        ...
        <div className="mt-6 border-t border-slate-100 pt-4">
          ...
          <div className="space-y-1">
            <Link href="/settings" className="flex items-center gap-3 rounded-lg px-4 py-2 text-slate-500 transition-colors hover:text-blue-700">
              <Sparkles size={20} />
            ...
            <button onClick={handleSignOut}
              className="flex w-full items-center gap-3 rounded-lg px-4 py-2 text-left text-slate-500 transition-colors hover:text-blue-700">
              <LogOut size={20} />
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute bottom-0 left-0 top-0 w-72 max-w-[85vw] border-r border-slate-200 bg-slate-50 p-4">
            <div className="absolute top-4 right-4">
              <button onClick={() => setSidebarOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100">
                <X size={20} />
              </button>
            </div>
            <div className="flex h-full flex-col pt-12">
              ...
              <div className="mt-4 px-2">
                <button onClick={() => setCoursesOpen(!coursesOpen)}
                  className="flex h-11 w-full items-center justify-between rounded-xl bg-slate-100 px-4 text-slate-700 transition-colors hover:bg-slate-200">
                  <span className="text-sm font-semibold tracking-tight">Select Course</span>
                  <ChevronDown size={18} />
                </button>
                {coursesOpen && (
              ...
              <div className="mt-6 border-t border-slate-200 px-2 pt-4">
                ...
                <div className="space-y-1">
                  <Link href="/settings" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-lg px-4 py-2 text-slate-500 transition-colors hover:text-blue-700">
                    <Settings size={20} />
                    <span className="text-sm font-medium">Settings</span>
                  </Link>
                  <button onClick={handleSignOut}
      ...
      <div className="flex min-h-screen flex-1 flex-col bg-[#f5f7f9] md:ml-64">
        <header className="fixed left-0 right-0 top-0 z-30 h-16 border-b border-slate-100 bg-white/95 backdrop-blur-xl md:left-64">
          <div className="flex h-full items-center justify-between gap-3 px-4 sm:px-5 md:px-8">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-blue-900">{pageTitle}</h2>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 md:gap-6">
              <div className="relative hidden md:block">
                <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input className="w-64 rounded-lg border-none bg-slate-100 py-2 pl-10 pr-4 text-sm transition-all focus:ring-2 focus:ring-blue-500" placeholder="Search notes, exams..." type="text" />
              </div>
