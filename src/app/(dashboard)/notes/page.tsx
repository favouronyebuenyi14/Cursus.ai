...
export default function NotesPage() {
  ...
  function goToRecorder(courseId?: string) {
    router.push(courseId ? `/recorder?course=${courseId}` : '/recorder')
  }
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
                    ...
                    <div className="relative">
                      ...
                      {isMenuOpen ? (
                        <MenuPanel onClick={event => event.stopPropagation()}>
                          ...
                          <MenuAction icon={<Mic size={16} />} label="Record note" onClick={() => goToRecorder(course.id)} />
                        </MenuPanel>
                      ) : null}
                    </div>
                  </div>
                ...
                <EmptyState
                  ...
                  actions={
                    <div className="flex flex-wrap items-center justify-center gap-3">
                      ...
                      <button onClick={() => goToRecorder(selectedCourse.id)} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#eef1f3] px-5 text-sm font-semibold text-[#2c2f31] transition-all hover:bg-slate-200 md:h-11">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 z-40 flex w-full items-center justify-around border-t border-slate-200 bg-white/90 py-3 backdrop-blur-xl">
        ...
        <button onClick={() => goToRecorder(selectedCourse?.id)} className="flex flex-col items-center gap-1 text-[#595c5e]"><Mic size={22} /><span className="text-[10px] font-bold">Record</span></button>
